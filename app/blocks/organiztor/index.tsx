        ...
        <div
            ...
            <div className={'grid grid-cols-2 px-24 items-start gap-6'}>
                ...
                <div className={'bg-[#efefef] h-full p-7 flex flex-col justify-center gap-5'}>
                    <p className={'font-bold text-4xl text-black'}>{name}</p>
                    <p className={'font-medium text-xl text-[#666]'}>{post}</p>
            ...
            <p className={'text-black uppercase font-bold text-xl w-1/3 text-center'}>Организовал следующие мероприятия</p>
            <p className={'text-cOrange font-bold text-5xl'}>Путешествия с ART PLANET</p>
            <div className={'flex flex-col gap-2 text-center'}>
                {tours.map((tour,counter)=>{
                    return(
                        <p key={counter} className={'font-light text-xl text-black list-disc'}>● {tour.name} - {tour.year}</p>
                    )
                })}
            </div>
        </div>
